...
geotab.addin.heatmap = () => {
  ...
  // Store last coordinates for display/download
  let lastHeatmapCoordinates = [];
  ...
  let displayHeatMapForLocationHistory = () => {
    ...
    api.multiCall(calls, function (results) {
      if (resultsEmpty(results)) {
        ...
        toggleLoading(false);
        lastHeatmapCoordinates = [];
        updateHeatmapDataDisplay();
        return;
      }      
      ...
      lastHeatmapCoordinates = coordinates.slice(); // Save for display/download
      updateHeatmapDataDisplay();
  ...
  let displayHeatMapForExceptionHistory = () => {
    ...
    api.multiCall(calls, function (results) {
      ...
      api.multiCall(calls, function (results) {
        ...
        lastHeatmapCoordinates = coordinates.slice();
        updateHeatmapDataDisplay();
  ...
  // --- UI for Display & Download Data ---
  let dataDiv, displayBtn, downloadBtn, tableBtn, tableDiv;

  function updateHeatmapDataDisplay() {
    // update raw JSON display
    if (dataDiv && dataDiv.style.display !== 'none') {
      dataDiv.textContent = JSON.stringify(lastHeatmapCoordinates, null, 2);
    }
  ...
  function renderHeatmapTable() {
    if (!tableDiv) return;
    tableDiv.innerHTML = '';
    if (!lastHeatmapCoordinates.length) {
      tableDiv.innerHTML = '<em>No heatmap data to display.</em>';
      return;
    }
    ...
    ['#', 'Latitude', 'Longitude', 'Value'].forEach(h => {
    ...
    lastHeatmapCoordinates.forEach((point, i) => {
      let tr = document.createElement('tr');
      [i+1, point.lat, point.lon, point.value].forEach(val => {
  ...
  let initializeInterface = coords => {
    ...
    // --- Add Show & Download Data Buttons ---
    displayBtn = document.createElement('button');
    displayBtn.textContent = 'Show Heatmap Data (JSON)';
    displayBtn.id = 'showHeatmapData';
    displayBtn.style.margin = '10px';
    displayBtn.onclick = function() {
      if (!lastHeatmapCoordinates.length) {
        alert('No heatmap data to display. Run a search first.');
        return;
      }
      // Toggle display
    ...
    downloadBtn.textContent = 'Download Heatmap Data (JSON)';
    downloadBtn.id = 'downloadHeatmapData';
    downloadBtn.style.margin = '10px';
    downloadBtn.onclick = function() {
      if (!lastHeatmapCoordinates.length) {
        alert('No heatmap data to download. Run a search first.');
        return;
      }
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(lastHeatmapCoordinates, null, 2));
      const dlAnchor = document.createElement('a');
      dlAnchor.setAttribute("href", dataStr);
      dlAnchor.setAttribute("download", "heatmap_data.json");
    ...
    tableBtn.textContent = 'Show Heatmap Data (Table)';
    tableBtn.id = 'showHeatmapDataTable';
    tableBtn.style.margin = '10px';
    ...
    tableDiv.id = 'heatmapDataTableDiv';
    tableDiv.style.maxHeight = '300px';
    tableDiv.style.overflowY = 'auto';
    tableDiv.style.background = '#fafafa';
    tableDiv.style.border = '1px solid #ccc';
    tableDiv.style.padding = '8px';
    tableDiv.style.marginTop = '8px';
    tableDiv.style.display = 'none';

    // Add buttons and data viewers to DOM (after the map)
    let container = document.getElementById('heatmap-map').parentNode;
